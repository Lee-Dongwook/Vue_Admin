...
onMounted(() => {
  chatRef.value.demo = {
    reponse: message => {
      console.log(message);
      return {
        text: "데모 전용입니다. AI 서비스가 필요한 경우 https://deepchat.dev/docs/connect를 참조하세요."
      };
    }
  };
});
...
<template>
  <deep-chat
    ...
    :textInput="{
      ...
      placeholder: { text: '', style: { color: '#bcbcbc' } }
    }"
    :messageStyles="{
      default: {
        shared: {
    ...
    :history="[
      { text: '한신은 누구인가?', role: 'user' },
      {
        text: '한신(약 기원전 231년~기원전 196년)은 진나라 말기와 한나라 초기의 뛰어난 군사 전략가이자 전략가였습니다. 그는 후세에 "불멸의 군인"으로 칭송받았습니다. ',
        role: 'ai'
      }
    ]"
    :demo="true"
